...
describe("balance endpoints", () => {
    ...
    test("should return correct balance when based on existing transactions with various currencies", async () => {
        ...
        // 44.1614838 +- 11.5555 = 32.6059838 usd
        ...
        // 21515.2 usd (crypto)
        await addTransaction(superTestRequest, { name: "Test Transaction 6", amount: 0.2, currency: "btc", assetType: "crypto", date: "2022-01-07" });
        // 31463.56 usd (crypto)
        await addTransaction(superTestRequest, { name: "Test Transaction 7", amount: 4, currency: "eth", assetType: "crypto", date: "2022-01-07" });
        // 31491.98085 usd (crypto)
        await addTransaction(superTestRequest, { name: "Test Transaction 8", amount: 50, currency: "ada", assetType: "crypto", date: "2022-01-07" });
        // 31488.570348 usd (crypto)
        await addTransaction(superTestRequest, { name: "Test Transaction 9", amount: -6, currency: "ada", assetType: "crypto", date: "2022-01-07" });

        // 2068.65 usd (stock)
        await addTransaction(superTestRequest, { name: "Test Transaction 10", amount: 15, currency: "AMD", assetType: "stock" });
        // 427.1 usd (stock)
        await addTransaction(superTestRequest, { name: "Test Transaction 10", amount: 2, currency: "AAPL", assetType: "stock" });
